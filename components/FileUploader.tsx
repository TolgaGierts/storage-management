        ...
        return toast({
          description: (
            <p className='body2 text-white'>
              ...
              is too large. File size must be less than {MAX_FILE_SIZE / 1000000}MB
            </p>
  ...
  return (
    <div {...getRootProps()} className='cursor-pointer'>
      ...
      <Button type='button' className={cn('uploader-button', className)}>
        <Image src='/assets/icons/upload.svg' alt='file-upload' width={24} height={24} />
        <p>Upload</p>
      </Button>
      {files.length > 0 && (
          ...
          {files.map((file, index) => {
            ...
            return (
              <li key={`${file.name}-${index}`} className='uploader-preview-item'>
                {/* <Image src={URL.createObjectURL(file)} alt={file.name} width={36} height={36} /> */}
